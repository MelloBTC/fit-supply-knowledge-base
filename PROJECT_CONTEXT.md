...
The information-architecture pressure test now exists in `wiki/knowledge-base/information-architecture-pressure-test.md`. It compares the starter department-style folder structure against the `fast-os-capability-roadmap` lifecycle, readiness-state, and agent-capability evidence. The decision is to keep physical folders stable for now, use workflow-first navigation lenses, and promote new folders only when real navigation pressure or page density justifies migration. The first IA-guided navigation cleanup now makes those lenses explicit in `wiki/index.md` and `wiki/knowledge-base/index.md`.
