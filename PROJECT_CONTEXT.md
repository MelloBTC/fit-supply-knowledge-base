...
The proposal package readiness synthesis lane now exists in `wiki/sales/proposal-package-readiness.md`. It is a `target-state` page that classifies project-context readiness, room context and uncertainty, existing equipment/trade-in capture, design asset readiness, ECDESIGN/rendering review, quote/proposal package alignment, vendor branches, interactive proposal parity, proposal delivery, and follow-up state through the lifecycle, capability, readiness/state, evaluation/build-readiness, storage-domain, and domain-promotion lenses. It does not import raw customer folders, quote PDFs, photos, pricing, contact details, generated proposal assets, or presentation-builder source artifacts, and it keeps autonomous proposal generation, visual generation, and customer-facing proposal sends out of scope.
...
The domain model review now exists in `wiki/knowledge-base/domain-model-review.md`. Its 2026-06-27 pressure recheck records `order-execution` as the first bounded promoted domain after Josh approved a pilot migration, keeps `source-intelligence`, `lead-intake`, `hunter-sales`, `quote-readiness`, and `proposal-readiness` as active sales sub-lanes, and defers top-level domains for `agent-workflows`, `operating-model`, `lead-intelligence`, `quote-readiness`, `proposal-readiness`, `installed-base`, and `accounts-and-sites` until compiled page density or validated examples support the move.
