...
If evaluation discipline becomes the priority, review FSCR-011 for order-execution gate fixtures or the FSCR-012 fixture bundle for hunter-sales policy evaluation. Otherwise, continue the example-first rule before build-facing requirements: capture downstream outcome evidence for the formal-feed source-aware example, capture another explicit downstream lead-intake outcome, or capture a dealer-backed hunter-sales example from the open validation list before promoting any remaining policy defaults. If no dealer examples are available, continue with a non-gated roadmap workflow family.
